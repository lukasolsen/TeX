...
| TEX-H-001 | Medium | frontend test matrix | No DOM/component accessibility test existed for primary open-project, workspace, editor, build, search, or PDF workflows. Semantic regressions, missing names, focus loss, and status-announcement defects could pass all prior automated gates. | §5 Interaction and accessibility | Add user-centric JSDOM workflow coverage and axe-core scans; retain real-browser keyboard, screen-reader, contrast, and forced-colors qualification because simulated automation is incomplete. | `e724ea0` | Fixed |
...
| TEX-H-003 | Medium | `build-configuration-dialog.tsx` | Environment text was parsed on every keystroke and immediately reserialized. An incomplete but valid-in-progress `NAME=value` entry disappeared before the user could type `=value`. Save also permitted duplicate submission and could close the dialog while validation was pending. | §3 State ownership; §5 Truthful interaction | Preserve raw draft text, validate complete lines only on save, bound fields, synchronously exclude duplicate saves, and prevent dismissal until the owned save settles. | `e724ea0` | Fixed |
| TEX-H-004 | Medium | shared UI components; `src/index.css`; build/settings workflows | The build-profile combobox had no accessible name, settings skipped a heading level, command-dialog title/description were outside the popup, every alert was assertive, and several semantic surface tokens/custom accents escaped forced-colors mapping. Focus rings also used low-contrast neutral tokens. | §5 Interaction and accessibility | Correct names and heading structure; bind dialog metadata inside its popup; reserve assertive alerts for destructive failures; complete system-color mappings, color-scheme declarations, and focus tokens; exercise primary workflows with role/name assertions and axe. | `e724ea0` | Fixed |
| TEX-H-005 | Medium | fixture and asset inventory | Committed LaTeX build databases and `.fls` files contained local absolute paths/timestamps but were not test inputs. Root-level app artwork and the Vite logo had no runtime, bundle, or packaging consumer. The fixture README incorrectly claimed no generated bytes were committed despite an intentional PDF/SyncTeX test pair. | §3 Fixtures and dependency direction; Phase 5 removal | Remove incidental outputs and dead assets; preserve only byte-contract fixtures; state exact provenance/ownership and verify the full Rust fixture/PDF suite after deletion. | `e724ea0` | Fixed |
| TEX-H-006 | Medium | `clean-auxiliary-dialog.tsx` | Preview and clean completions had no request identity, duplicate clean clicks were not synchronously excluded, and an unmounted dialog could receive stale completion. | §3 Ordering; §5 React effects | Give preview/clean operations a monotonic owner, reject duplicate destructive requests before state propagation, suppress stale completion, and route event promises through the detached-task owner. | `e724ea0` | Fixed |
...
- `e724ea0`: added file-scoped JSDOM, Testing Library, user-event, and axe-core
  coverage for project entry, toolbar, editor/search, build/PDF, and settings
  workflows; corrected accessible names, heading order, dialog relationships,
  live-region priority, forced-color tokens, focus rings, and browser color
  schemes; preserved build-configuration drafts and serialized destructive
  dialog operations; declared exported shared-component returns; and removed
  dead artwork plus non-portable generated fixture output. The four additions
  are development-only test dependencies; runtime permissions, IPC, and schemas
  did not change. Oxlint, project-reference typechecking, 74 frontend tests,
  production build, Rust formatting, Clippy with warnings denied, and 66 Rust
  tests passed. JSDOM color contrast is deliberately excluded under the
  tool-maintainer limitation and remains a packaged-app qualification item.
