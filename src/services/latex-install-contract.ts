...
export function parseInstallProgress(value: unknown): InstallProgress {
  ...
  return {
    ...
    activeStep: nullableInteger(input.activeStep, "active step", 0, MAX_STEPS),
    startedAt: integer(
      input.startedAt,
...
function parseManualInstruction(value: unknown): ManualInstruction {
  ...
  return {
    summary: nonEmptyString(input.summary, "manual instruction summary", 1_024),
...
function parseInstallOption(value: unknown): InstallOption {
  ...
  return {
    ...
    elevation: enumValue(input.elevation, "installation elevation", ELEVATIONS),
...
function parseInstallStepState(value: unknown): InstallStepState {
  ...
  return {
    ...plan,
    status: enumValue(input.status, "installation step status", STEP_STATUSES),
    detail: nullableString(input.detail, "installation step detail", 1_024),
  }
}
